...
let ArryofListWithSumOfColors = [];

List.map((grp) => {
    let newArr = {...ListWithSumOfColors};
    for (let i = 0; i < grp.length; i++) {
        if (i === 0) {
            newArr.id = Number(grp[i].match(/\d+/g)[0]);
        }
        if (grp[i].match(/red/g)) {
            let newRed = Number(grp[i].match(/\d+/g)[0]);
            if(newRed > newArr.red){
                newArr.red = newRed;
            }
            
        }else if(grp[i].match(/blue/g)){
            let newBlue = Number(grp[i].match(/\d+/g)[0]);
            if(newBlue > newArr.blue){
                newArr.blue = newBlue;
            }
        }else if(grp[i].match(/green/g)){
            let newGreen = Number(grp[i].match(/\d+/g)[0]);
            if(newGreen > newArr.green){
                newArr.green = newGreen;
            }
        }
    }
    ArryofListWithSumOfColors.push(newArr)
})


ArryofListWithSumOfColors.map(grp => {
    let newMulti = grp['red'] * grp['blue'] * grp['green'];
    grp['multiplicator'] = newMulti;
})

let SumOfAllMultis = 0;

ArryofListWithSumOfColors.map(grp => {
    SumOfAllMultis += grp['multiplicator'];
})

console.log("SumOfAllMultis", SumOfAllMultis)
