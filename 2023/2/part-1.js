...
List.map((grp) => {
    ...
    for (let i = 0; i < grp.length; i++) {
        ...
        if (grp[i].match(/red/g)) {
            ...
            if(Number(grp[i].match(/\d+/g)[0]) > 12){
                newArr.red = -1;
            }
            
        }else if(grp[i].match(/blue/g)){
            if(Number(grp[i].match(/\d+/g)[0]) > 14){
                newArr.green = -1;
            }
        }else if(grp[i].match(/green/g)){
            if(Number(grp[i].match(/\d+/g)[0]) > 13){
                newArr.blue = -1;
            }
        }
    }
...
console.log("ArryofListWithSumOfColors", ArryofListWithSumOfColors)

let sumOfIdsPossibles = 0;

ArryofListWithSumOfColors.map(grp => {
    if(grp['red'] !== -1 && grp['blue'] !== -1 && grp['green'] !== -1){
        sumOfIdsPossibles += grp['id'];
    }
})

console.log("sumOfIdsPossibles", sumOfIdsPossibles)