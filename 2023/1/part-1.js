...
function isPlus3MoreNumb(num) {
    return /^\d{3}$/.test(num);
}

const ArrayNumberFake = ArrayInput.map((group) => [...group]).map(group2 => group2);

const NewArr = ArrayNumberFake.map((grp) => grp.filter(e => !isAlphanumeric(e))).map(map => {
    let newArray = []
    if(map.length > 2){
        newArray.push(map[0]);
        newArray.push(map[map.length-1]);
        return newArray;
    }
    ...
    return map;
}).map(map => map.join("")).map(Number);

const SumOfAll = NewArr.reduce((acc, val) => acc + val,0)
