...
function isAlphanumericOrDott(str) {
    return /^[0-9]|\.|\n+$/.test(str);
}

const ArrayList = Input.split('\n');

let ListOfNumbersToSum = [];

ArrayList.map(arr => [arr]);

function ifSymbolAdd(symbol, number) {
    if (symbol) {
        ListOfNumbersToSum.push(Number(number));
    }
}

for (let k = 0; k < ArrayList.length; k++) {
    if (k === 0) { //primeira linha
        let Line = ArrayList[k].matchAll(/\d+/g);
        for (const match of Line) {
            let destructedArray = ArrayList[k].split("");
            let nextLineArry = ArrayList[k + 1].split("");
            /* console.log(
                `Found ${match[0]} start=${match.index} end=${match.index + match[0].length - 1}.`,
            ); */
            if (match.index === 0) { //primeira posicao vai só à seguinte
                ifSymbolAdd(!isAlphanumericOrDott(destructedArray[match.index + match[0].length]), match[0]);
                for (let g = 0; g < match[0].length + 1; g++) {
                    //verifica linhas abaixo com diagonal
                    ifSymbolAdd(!isAlphanumericOrDott(nextLineArry[g]), match[0]);
                }
            } else {
                //vai à da frente
                if ((match.index + match[0].length) !== nextLineArry.length) {//vai à da frente se houve mais caracteres na frente
                    ifSymbolAdd(!isAlphanumericOrDott(destructedArray[match.index + match[0].length]), match[0]);
                    ifSymbolAdd(!isAlphanumericOrDott(nextLineArry[match.index + match[0].length]), match[0]);
                }
                //vai a trás
                ifSymbolAdd(!isAlphanumericOrDott(destructedArray[match.index - 1]), match[0]);
                //verifica linhas abaixo com diagonal
                for (let g = match.index -1 ; g < match.index + match[0].length; g++) {
                    ifSymbolAdd(!isAlphanumericOrDott(nextLineArry[g]), match[0]);
                }
            }
        }
        ...
        let Line = ArrayList[k].matchAll(/\d+/g);
        let destructedArray = ArrayList[k].split("");
        let lineBefore = ArrayList[k - 1].split("");
        for (const match of Line) {
            if (match.index === 0) { //primeira posicao vai só à seguinte
                ifSymbolAdd(!isAlphanumericOrDott(destructedArray[match.index + match[0].length]), match[0]);
                for (let g = 0; g < match[0].length + 1; g++) {
                    //verifica linhas acima com diagonal
                    console.log("match[0].length",match[0].length)
                    console.log("lineBefore[g]", lineBefore[g])
                    ifSymbolAdd(!isAlphanumericOrDott(lineBefore[g]), match[0]);
                }
            } else {
                //vai há frente
                if ((match.index + match[0].length) !== destructedArray.length) { //vai à da frente se houve mais caracteres na frente
                    ifSymbolAdd(!isAlphanumericOrDott(destructedArray[match.index + match[0].length]), match[0]);
                    ifSymbolAdd(!isAlphanumericOrDott(lineBefore[match.index + match[0].length]), match[0]);
                }
                //vai a trás 
                ifSymbolAdd(!isAlphanumericOrDott(destructedArray[match.index - 1]), match[0]);
                for (let g = match.index -1 ; g < match.index + match[0].length; g++) {
                    //verifica linhas acima
                    ifSymbolAdd(!isAlphanumericOrDott(lineBefore[g]), match[0]);
                }
            }
        }
        ...
        for (const match of Line) {
            if (match.index === 0) { //primeira posicao vai só à seguinte
                ...
                //vai à da frente se houve, tanto na linhas como na anterior ou posterior
                if ((match.index + match[0].length) < nextLineArry.length) { //vai à da frente se houve mais caracteres na frente
                    ifSymbolAdd(!isAlphanumericOrDott(destructedArray[match.index + match[0].length]), match[0]);
                    ifSymbolAdd(!isAlphanumericOrDott(nextLineArry[match.index + match[0].length]), match[0]);
                    ifSymbolAdd(!isAlphanumericOrDott(lineBefore[match.index + match[0].length]), match[0]);
                ...
                ifSymbolAdd(!isAlphanumericOrDott(destructedArray[match.index - 1]), match[0]);
                for (let g = match.index -1 ; g < match.index + match[0].length; g++) {
                    //verifica linhas abaixo e cim com diagonal
                    ifSymbolAdd(!isAlphanumericOrDott(nextLineArry[g]), match[0]);
                    ifSymbolAdd(!isAlphanumericOrDott(lineBefore[g]), match[0]);
                }
            }
...
console.log(ListOfNumbersToSum)
const Sum = ListOfNumbersToSum.reduce((acc, val) => acc + val, 0)
console.log(Sum)